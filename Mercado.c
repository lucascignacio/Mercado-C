...
void menu(){
	...
	int opcao;
	scanf("%d", &opcao);
	getchar(); // Para não pular essa opção
...
void comprarProduto(){
	if(contador_produto > 0){
			...
			if(produtos[i].codigo == codigo){
				...
				if(contador_carrinho > 0){
					...
					if(retorno[0] == 1){
						...
						printf("Aumentei a quantidade do produo %s já existente no carrinho.\n",
					...
					carrinho[contador_carrinho].quantidade = 1;
					contador_carrinho++;
					printf("O produto %s foi adicionado ao carrinnho.\n", strtok(p.nome, "\n"));
					sleep(2);
					menu();
				}
			}
		...
		printf("Ainda não existe produtos para vender.\n");
		sleep(2);
		menu();
	}
}
...
void visualizarCarrinho(){
	if(contador_carrinho > 0){
		...
		printf("Não tem produtos no carrinho.\n");
		sleep(2);
		menu();
	}
}
